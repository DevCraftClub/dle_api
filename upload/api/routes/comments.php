	...
	$this->get('[/]', function (Request $request, Response $response, array $args) use ($possibleData, $api_name, $connect, $header, $access) {
		...
		if ($access['full'] || $access['can_read']) {
			...
			foreach ($header as $data => $value) {
				...
				if (!$access['full'])
					if (in_array($data, ['user_id', 'autor']) && $access['own_only']['access']) continue;
				if ($keyData !== false) {
			...
			if (!$access['full']) {
				if (strlen($possibleParams) === 0 && $access['own_only']['access'])
					$possibleParams .= " WHERE user_id = {$access['own_only']['user_id']} OR autor = '{$access['own_only']['user_name']}'";
				else $possibleParams .= " AND (user_id = {$access['own_only']['user_id']} OR autor = '{$access['own_only']['user_name']}')";
			}
