...
$requestPmPlaceholders = ['{%user_id%}', '{%username%}', '{%level%}', '{%request_id%}', '{%request_url%}', '{%subject%}'];
$decisionPmPlaceholders = ['{%user_id%}', '{%username%}', '{%api_key%}', '{%subject%}'];
$denyPmPlaceholders = ['{%user_id%}', '{%username%}', '{%subject%}'];
...
return FormSchemaBuilder::create('dleapi')
	->layout(FormLayout::TABS)
		...
		->section(__('Уведомления о заявках'))
			...
			->editor('pm_request_body', __('PM: текст заявки'))
		...
		->section(__('Уведомления о решении'))
			...
			->editor('pm_approve_body', __('PM: текст одобрения'))
			...
			->editor('pm_deny_body', __('PM: текст отказа'))
		...
		->section(__('Email-шаблоны'))
			...
			->codeViewer('email_request_body', __('Email: тело заявки'))
			...
			->codeViewer('email_decision_body', __('Email: тело решения'))
