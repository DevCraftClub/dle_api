...
(function (global) {
	...
	function boot() {
		...
		function setButtons(state) {
			if (requestBtn) {
				requestBtn.hidden = !state.request;
				requestBtn.style.display = state.request ? '' : 'none';
			}
			if (newBtn) {
				newBtn.hidden = !state.renew;
				newBtn.style.display = state.renew ? '' : 'none';
			}
			if (approveBtn) {
				approveBtn.hidden = !state.approve;
				approveBtn.style.display = state.approve ? '' : 'none';
			}
			if (denyBtn) {
				denyBtn.hidden = !state.deny;
				denyBtn.style.display = state.deny ? '' : 'none';
			}
		}
		...
		function render(data) {
			...
			if (currentStatus === 'approved' && data.key) {
				...
				if (keyInput) {
					var plain = String(data.key.key || '');
					var showPlain = !!data.viewer_is_admin || data.mode === 'self';
					keyInput.value = showPlain && plain ? plain : (data.key.masked_key || mask(plain));
					keyInput.dataset.realKey = plain;
					if (showPlain) {
						keyInput.classList.remove('dleapi-profile-key__masked');
					} else {
						keyInput.classList.add('dleapi-profile-key__masked');
					}
				}
		...
		if (copyBtn && keyInput) {
			copyBtn.addEventListener('click', function () {
				var text = String(keyInput.dataset.realKey || '');
				if (!text) {
					return;
				}
				copyText(text).then(function (ok) {
					...
					setText(notice, t('Не удалось автоматически скопировать ключ.'));
				});
			});
		}
